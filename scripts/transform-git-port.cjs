...
const roots = [path.join(DST, 'src/main/git'), path.join(DST, 'src/main/github')];
const singles = [
  path.join(DST, 'src/main/operations/OperationCoordinator.ts'),
  path.join(DST, 'src/main/operations/OperationRegistry.ts'),
  path.join(DST, 'src/main/projects/SnapshotCache.ts'),
];
...
function transform(src) {
  ...
  t = t.replace(/import type \{([^}]+)\} from '@shared\/contracts\/operations';/g, (full, inner) => {
...
console.log('Transformed', n, '/', files.length);
console.log('lines', fs.readFileSync(path.join(DST, 'src/main/git/GitMutationService.ts'), 'utf8').split(/\n/).length);
